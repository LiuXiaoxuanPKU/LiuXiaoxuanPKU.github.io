...
3. **There are many spare FLOPs in the serving system** "Spare FLOPs" refer to the computational capacity that remains unused during regular operations because the LLM serving systems are usually provisioned with more resources than necessary for the average workload. Since computational workload can vary significantly based on user demand and the complexity of the queries, when the service traffic is not on spike, these spare FLOPs are untapped computational power that can be used for additional tasks without scarificing user experience. In our case, sparse FLOPs can be used to perform gradient updates on the draft model to adapt for query distribution shifts.
