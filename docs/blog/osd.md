...
During each generation, there's a need to transfer the model from HBM (High Bandwidth Memory) to SRAM (Static Random Access Memory) and access the key-value cache of all previously generated tokens. This procedure places a substantial demand on memory bandwidth, subsequently resulting in the suboptimal utilization of the GPU. The problem is more conspicuous for the larger target model and leads to a longer inference latency. Speculative decoding aims at transfering the majority of workload to a much smaller draft model. The draft model retains the autoregressive nature, generating tokens one at a time but with a significantly faster speed. The target model can validate multiple generated tokens from the draft model in a single forward pass (refer to [the paper](https://arxiv.org/abs/2211.17192) for more detail). 
...
### Key observations pave the path toward improved speedup
Performance of the original speculative decoding algorithm depends heavily on one or a set of reliable draft models. In practice, open-domain draft models has poor speculation accuracy. Yet it's also hard to predict query distributions and prepare specialized draft models offline to ensure speculation accuracy. To address the eixsiting challenges and seek new ways to improve speculative decoding in real LLM serving systems, we present several interesting observations:
<p align="center">
...
3. **There are many spare FLOPs in the serving system for distillation**. Spare FLOPs' refer to the unused computational capacity during regular operations, primarily because (1) LLM inference is not computationally demanding, and (2) LLM serving systems are often provisioned with resources exceeding the needs of the average workload.
